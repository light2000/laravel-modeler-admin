<?php

namespace Modules\User\Http\Controllers\Options;

use App\Http\Resources\OptionCollection;
use Illuminate\Http\Request;
use Modules\User\Http\Controllers\UserController;
use Modules\User\Models\Role;

class RoleOptionsController extends UserController
{
    public function index(Request $request)
    {
        $request->validate([
            'for' => 'required|string',
            'id' => 'nullable|integer',
            'ids' => 'nullable|string',
            'q' => 'nullable|string',
            'page' => 'nullable|integer|min:1',
            'page_size' => 'nullable|integer|min:1|max:100',
        ]);

        $for = $request->input('for');
        $id = $request->input('id');
        $ids = $request->input('ids');
        $keyword = $request->input('q');
        $pageSize = $request->input('page_size', config('pagination.default_page_size'));

        $config = [
            'administrator.role_ids' => [
                'name',
            ],
        ];

        if (! isset($config[$for])) {
            abort(400, "Invalid 'for' parameter: {$for}");
        }

        $displayAttrs = $config[$for];

        $query = Role::query();

        if (! empty($ids)) {
            $idList = array_values(array_filter(array_map('intval', explode(',', $ids))));
            if ($idList !== []) {
                $query->whereIn('id', $idList);
            }
        } elseif (! empty($id)) {
            $query->where('id', $id);
        } elseif (! empty($keyword) && ! empty($displayAttrs)) {
            $query->where(function ($q) use ($displayAttrs, $keyword) {
                foreach ($displayAttrs as $index => $attr) {
                    if ($index === 0) {
                        $q->where($attr, 'like', "%{$keyword}%");
                    } else {
                        $q->orWhere($attr, 'like', "%{$keyword}%");
                    }
                }
            });
        }

        $paginator = $query->paginate($pageSize);

        $paginator->getCollection()->transform(function ($item) use ($displayAttrs) {
            $labelParts = [];
            foreach ($displayAttrs as $attr) {
                $value = $item->{$attr};
                if ($value !== null && $value !== '') {
                    $labelParts[] = $value;
                }
            }

            return [
                'value' => $item->id,
                'label' => ! empty($labelParts) ? implode('/', $labelParts) : '#'.$item->id,
            ];
        });

        return new OptionCollection($paginator);
    }
}
